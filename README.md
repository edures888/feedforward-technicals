...
Processed final split, similar to "MIND small" dataset scale: 88K users, 176K impressions, average history length 39. Note this was matched to MIND's format where only the final impression is used, and users' histories only include past clicks (negatives discarded). The alternative would be to utilise past impression windows too.
...
Item pooling methods: Mean, TopK, TopK + Mean, SASRec Layer, Deep Interest Network Attention (DIN Attention).
...
| Bi-Encoder all components | 0.7044 | 0.3497 | 0.3869 | 0.8367 | 0.6217 | 0.6510 |
| Half Cross-Encoder all components | **0.7100** | **0.3529** | **0.3928** | - | - | - |
...
|--------------|----------|------------|----------|
| Bi-encoder | 0.6786 | 0.3309 | 0.3646 |
| Bi-Encoder - ColBERT | 0.6705 | 0.3261 | 0.3578 |
| Half Cross-Encoder | 0.6871 | 0.3380 | 0.3745 |
| Half Cross-Encoder - ColBERT | 0.6788 | 0.3330 | 0.3682 |

Table 2: Comparisons with alternate architectures, all trained with InfoNCE with multiple positives, no additional components

| Configuration | MIND AUC | MIND MRR  | MIND NDCG@5 | Mix AUC | Mix MRR  | Mix NDCG@5 |
|--------------|----------|------------|----------|------------|----------|-|
| Bi-encoder MINCE Loss | 0.6786 | 0.3309 | 0.3646 | 0.7933 | 0.5356 | 0.5645 |
| + MaxSim / ColBERT | 0.6705 | 0.3261 | 0.3578 | - | - | - |
| + LoRA only | 0.6891 | 0.3372 | 0.3728 | - | - | - |
| + LoRA, trainable base | 0.6906 | 0.3406 | 0.3761 | - | - | - |
| + SASRec Pool | 0.6526 | 0.3041 | 0.3366 | - | - | - |
| + Mix Pool (TopK + Mean) | 0.6836 | 0.3373 | 0.3721 | - | - | - |
| + DIN Attention Pool | 0.6880 | 0.3397 | 0.3735 | - | - | - |
| + Redundancy Penalty | 0.6855 | 0.3396 | 0.3740 | - | - | - |
| + All components | **0.6971** | **0.3478** | **0.3852** | 0.8287 | 0.6069 | 0.6363 |

Table 3: Additions to Bi-Encoder on Mind Small Validation set (73K impressions). Larger batch sizes of 48 used. Learning rates lightly swept and set higher at 1e-4 to match LoRA's 5e-4. `All components` run involved using more performant `Mix` pooling over `Mix`/`SASRec`.
